...
def create_mask(img_ch, threshold=0.1):
    ...
    """
    if img_ch.shape[1] == 2:
        noisy_img_abs_norm = normalize(abs(dualchannels2complex(img_ch)))
    else:
        noisy_img_abs_norm = normalize(img_ch[:, 0])
    mask = np.empty(noisy_img_abs_norm.shape)
    for k in range(noisy_img_abs_norm.shape[0]):
        orig_imgs_norm_threshold = np.where((noisy_img_abs_norm[k]) > threshold, 1, 0)
        ...
        if mask_background_1.sum() > 500: # to avoid a null mask in case the slice is noise everywhere
            mask_background_2 = mask_background_1
        else:
